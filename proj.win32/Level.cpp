...
vector<Rect> Level::getCollisionTilesY(Point point, int direction) {
	...
	TMXLayer *hazards = map->getLayer("hazards");
	...
	for (int b = -1; b < 2; b++) {
		...
		Sprite *tale = hazards->getTileAt(Point((int)point.x + b, mapheight - ((int)point.y + direction)));
		if (tile != NULL) {
...
vector<Rect> Level::getHazardsX(Point point, int direction) {
	...
	for (int b = -1; b < 2; b++) {
		...
		Sprite *tile = hazards->getTileAt(Point((int)point.x + direction, mapheight - ((int)point.y + b)));

		if (tile != NULL) {

			Rect tileRect = Rect();
			
			Point tmp = hazards->positionAt(Point((int)point.x + direction, mapheight - ((int)point.y + b)));
			/*tmp = hazards->positionAt(Point((int)point.x + b, mapheight - ((int)point.y + direction)));*/
			tileRect.setRect(tmp.x * SCALE_FACTOR, tmp.y * SCALE_FACTOR, map->getTileSize().width * SCALE_FACTOR, map->getTileSize().height * SCALE_FACTOR);
...
vector<Rect> Level::getHazardsY(Point point, int direction) {
	...
	for (int b = -1; b < 2; b++) {
		...
		Sprite *tile = hazards->getTileAt(Point((int)point.x + b, mapheight - ((int)point.y + direction)));

		if (tile != NULL) {

			Rect tileRect = Rect();

			Point tmp = hazards->positionAt(Point((int)point.x + b, mapheight - ((int)point.y + direction)));
			/*tmp = hazards->positionAt(Point((int)point.x + b, mapheight - ((int)point.y + direction)));*/
			tileRect.setRect(tmp.x * SCALE_FACTOR, tmp.y * SCALE_FACTOR, map->getTileSize().width * SCALE_FACTOR, map->getTileSize().height * SCALE_FACTOR);
...
vector<Rect> Level::getCollisionTilesX(Point point, int direction) {
	...
	for (int b = -1; b < 2; b++) {
		...
		Sprite *tale = hazards->getTileAt(Point((int)point.x + direction, mapheight - ((int)point.y + b)));
		if (tile != NULL) {
